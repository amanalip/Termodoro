...
Official Website & Knowledge Base:
...
Termodoro includes **311 automated Rust tests** and **41 automated Playwright web E2E tests**:
...
- **Timer Engine (`src/timer.rs`, 29 tests)**: Tests 24-cycle state machine progression, underflow safety on sub-second ticks, tuple time formatting, large duration formatting (up to 120 mins), 50 rapid skips, zero-duration progress calculations, pause, toggle, reset transitions, skip-no-credit semantics, and phase title/emoji parity.
- **Task Management (`src/tasks.rs`, 32 tests)**: Tests UUID generation uniqueness across 100 tasks, 500-task high volume benchmarks, dynamic filter index clamping plus stale-cursor healing in toggle/remove/navigation, transient JSON exclusions, boundary deletions, empty manager resilience, selection wrapping, multiline sanitization, and active task auto-reassignment.
- **Productivity Analytics (`src/stats.rs`, 34 tests)**: Tests 366-day leap year streaks, multi-day streaks across year and month boundaries, midnight-rollover session attribution, DST spring-forward/fall-back weekend continuity, 1,000-session large accumulation, minute-to-hour calculations, session metadata retention, empty window distributions, weekday histogram labels, and DST-safe, midnight-race-deterministic streak boundaries.
- **Storage & Zero-Telemetry Privacy (`src/storage.rs`, 28 tests)**: Tests atomic save/load roundtrips with unique per-save `.tmp` staging names (concurrent writers can never corrupt each other), corrupt-file quarantine (renamed aside with a timestamp instead of overwritten), hostile state-file inputs (invalid UTF-8 bytes, BOM prefixes, wrong-typed fields, duplicate keys, over-deep nesting), failed-save protection of previous data, out-of-range config sanitization on load, zero-telemetry schema invariants, rejection of third-party network SDKs/URLs, and local-only XDG directory isolation.
- **Application Workflows (`src/app.rs`, 52 tests)**: Tests 1,000-keystroke chaos fuzzing, 18-theme forward/backward navigation and disk persistence, exhaustive 9-row settings clamping welded to the config range constants, save-failure warning banner surfacing, modal input isolation and rapid editing/backspace (including ZWJ grapheme behavior), full 24-cycle E2E workflows, direct numeric tab jumping, modal dismissal keys, sound & desktop notification flags, status message expiration, sub-second tick accumulation, keypress/tick decoupling, quit-modifier semantics, and keybinding dispatchers.
- **Themes & Palettes (`src/theme.rs`, 16 tests)**: Tests all 18 palettes, WCAG relative luminance contrast formulas, forward/backward index cycling, phase color distinctness (no long-break color collides with an accent), byte-level RGB constraints, tolerant deserialization of unknown theme names, and serde roundtrips.
...
- **Help Popup Layout Math (`src/ui/help_popup.rs`, 6 tests)**: Tests `centered_rect` containment across geometry/percentage matrices, >100% clamp safety (previously a debug-build underflow panic), full-coverage behavior, zero-sized parents, margin symmetry within rounding, and area monotonicity.
- **Timer View Cycle Dots (`src/ui/timer_view.rs`, 6 tests)**: Tests the extracted dot-state machine (completed/active/upcoming per phase), interval clamping to 1..=24 against corrupt values, and pins the documented display desync when the interval shrinks below the live cycle position.
- **Settings View Consistency (`src/ui/settings_view.rs`, 1 test)**: Welds the human-readable range hints rendered in the Settings table to the single-source-of-truth constants in `config.rs`, so display text can never silently drift from enforced clamps.
- **Binary Event Loop (`src/main.rs`, 7 tests)**: Tests the extracted catch-up tick reconciliation (interval rounding, capped fired ticks, u32 saturation on absurd stalls instead of panics) and the Press/Repeat/Release key-kind dispatch filter.
...
- **Property-Based Suite ([`tests/property_tests.rs`](tests/property_tests.rs), 10 tests)**: proptest-generated randomized inputs (hundreds of cases per property, automatic shrinking of failures): Config/AppData serde roundtrips over arbitrary values, sanitize idempotence and range totality, timer state-machine invariants under random op sequences (remaining ≤ total, ratio ∈ [0,1], skips never credit progress), task cursor validity under random operation interleavings, and a deserializer that must never panic on arbitrary or JSON-flavored hostile input.
